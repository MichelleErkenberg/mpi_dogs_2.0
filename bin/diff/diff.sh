...
python3 bin/diff/extract_sequences.py "$OUTDIR/combined_pub.ref.trimmed.aln" bin/diff/related.txt "$OUTDIR/related_seq.fasta"
...
python3 bin/diff/aln_to_csv.py "$OUTDIR/related_seq.aln" "$OUTDIR/related_seq.csv"

#creates a .csv-file where N's are replaced by bases from closely related dogs (if they are the same)
python3 bin/diff/replace_n.py bin/names.txt "$OUTDIR/related_seq.csv" "$OUTDIR/replaced_seq.related_n.csv"

#uses the replaced_n.csv to create a new .fasta-file with replaced n's
python3 bin/diff/replaced_n_fasta.py "$OUTDIR/replaced_seq.related_n.csv" "$OUTDIR/replaced_seq.related_n.fasta"  

#write the mpi dog sequences with replaced n's into a new fasta 
seqkit grep -f bin/names.txt "$OUTDIR/replaced_seq.related_n.fasta" > "$OUTDIR/replaced_seq.related_n.mpi_dogs.fasta"

#add the ref dog to this fasta
cat "$MPI_BASE_PATH/data/science_dogs/Canis_lupus_familiaris.fasta" "$OUTDIR/replaced_seq.related_n.mpi_dogs.fasta" > "$OUTDIR/replaced_seq.related_n.mpi_dogs.added_ref.fasta"
